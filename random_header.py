...
class RandomHeader:
	...
	@classmethod
	def headers_available(cls):
		return cls._headers.keys() 

	@classmethod
	def get(cls, htype:str=None):
		keys = list(cls._headers.keys())
		
		if htype==None: htype = keys[random.randint(0, len(keys)-1)]
		
		_list = cls._headers[ htype ]
		_header = _list[ random.randint(0, len(_list)-1) ]
